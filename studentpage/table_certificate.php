                ...
                <table class="table table-dark display" id="myTable">
                  <thead>
                    <tr>
                      ...
                      $stmt1 = $conn->prepare("SELECT * FROM category_tbl WHERE cID = :id");
                      $stmt1->bindparam(":id", $id);
                      $stmt1->execute();
                      $row3 = $stmt1->fetch(PDO::FETCH_ASSOC);


                      $stmt = $conn->prepare("SELECT * FROM cert_tbl WHERE cCategory = :categ");
                      $stmt->bindparam(":categ", $row3['cCategory']);
                      $stmt->execute();
                              ...
                              <div class="modal-dialog" role="document" style="margin-right: 500px;">
                                <div class="modal-content" style="height: 700px; width: 700px;">
                                  <div class="modal-header">
                                    <h5 class="modal-title" id="exampleModalLabel" style="color: black;"><?=$row1['cCertname']?></h5>
                                    <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                                      <span aria-hidden="true">&times;</span>
                                    </button>
                                  </div>
                                  <div class="modal-body mx-auto" style="color: black;">
                                     <img height="500" width="650" src="<?=$row1['cPhoto']?>">
                                  </div>
                                </div>
                              </div>
                            </div>
